...
/** @title LeonicornSwap Lottery.
 * @notice It is a contract for a lottery system using randomness provided
 * externally.
 */
contract LeonicornSwapLottery is
    ReentrancyGuard,
    ILeonicornSwapLottery,
    Ownable
{
    ...
    uint256 public maxPriceTicketInLeon = 50 ether;
    uint256 public minPriceTicketInLeon = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInLeon;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] leonPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInLeon;
        uint32 finalNumber;
    }
    ...
    event UpdateTicketPrice(uint256 minPrice, uint256 maxPrice);
    event UpdateMaxTicketsBuy(uint256 maxTicketsPerBuy);
     ...
     * @param _leonTokenAddress: address of the LEON token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _leonTokenAddress, address _randomGeneratorAddress) {
        leonToken = IERC20(_leonTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of LEON to this contract
        uint256 amountLeonToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInLeon,
            _ticketNumbers.length
        );

        // Transfer leon tokens to this contract
        leonToken.safeTransferFrom(
            address(msg.sender),
            address(this),
            amountLeonToTransfer
        );

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInLeon += amountLeonToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInLeonToTransfer
        uint256 rewardInLeonToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInLeonToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        leonToken.safeTransfer(msg.sender, rewardInLeonToTransfer);

        emit TicketsClaim(
            msg.sender,
            rewardInLeonToTransfer,
            _lotteryId,
            _ticketIds.length
        );
    }
    ...
    function drawFinalNumberAndMakeLotteryClaimable(
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInLeon) *
        ...
        // Calculate prizes in LEON for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].leonPerBracket[j] =
                ...
                // A. No LEON to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].leonPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId]
            .amountCollectedInLeon - amountToShareToWinners);

        // Transfer LEON to treasury address
        leonToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
        ...
        onlyOwnerOrInjector
    {
        ...
        leonToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInLeon += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInLeon >= minPriceTicketInLeon) &&
                (_priceTicketInLeon <= maxPriceTicketInLeon),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInLeon: _priceTicketInLeon,
            ...
            amountCollectedInLeon: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount)
        ...
        onlyOwner
    {
        require(_tokenAddress != address(leonToken), "Cannot be LEON token");
    ...
    /**
     * @notice Set LEON price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInLeon: minimum price of a ticket in LEON
     * @param _maxPriceTicketInLeon: maximum price of a ticket in LEON
     */
    function setMinAndMaxTicketPriceInLeon(
        uint256 _minPriceTicketInLeon,
        uint256 _maxPriceTicketInLeon
    ) external onlyOwner {
        require(
            _minPriceTicketInLeon <= _maxPriceTicketInLeon,
            "minPrice must be < maxPrice"
        );

        minPriceTicketInLeon = _minPriceTicketInLeon;
        maxPriceTicketInLeon = _maxPriceTicketInLeon;
        emit UpdateTicketPrice(_minPriceTicketInLeon, _maxPriceTicketInLeon);
    }
        ...
        onlyOwner
    {
        ...
        emit UpdateMaxTicketsBuy(_maxNumberTicketsPerBuy);
    }
     ...
     * @param _priceTicket price of a ticket (in LEON)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].leonPerBracket[_bracket];
        } else {
            return 0;
        }
    }
